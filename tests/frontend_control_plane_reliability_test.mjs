...
const statusConsole = read('frontend/src/components/ModelStatusConsole.tsx')
assert.match(statusConsole, /function isModelNameList\(value: unknown\): value is string\[\]/)
assert.match(statusConsole, /function isAvailableModel\(value: unknown\): value is AvailableModel/)
assert.match(statusConsole, /function isTokenGroup\(value: unknown\): value is TokenGroup/)
assert.match(statusConsole, /!modelsResponse\.ok \|\| !configResponse\.ok \|\| !groupsResponse\.ok/)
assert.match(statusConsole, /!Array\.isArray\(models\) \|\| !models\.every\(isAvailableModel\)/)
assert.match(statusConsole, /!isModelNameList\(configData\)/)
assert.match(statusConsole, /!Array\.isArray\(groups\) \|\| !groups\.every\(isTokenGroup\)/)

const embed = read('frontend/src/components/ModelStatusEmbed.tsx')
assert.match(embed, /function isEmbedModelNameList\(value: unknown\): value is string\[\]/)
assert.match(embed, /if \(!isEmbedModelNameList\(data\.data\)\)/)
assert.doesNotMatch(embed, /Array\.isArray\(data\.data\) && data\.data\.length > 0/)
assert.match(embed, /customGroupsPayload\.every\(isEmbedCustomGroup\)/)
...
assert.match(embed, /isEmbedModelNameList\(group\.models\)/)
assert.match(embed, /TOKEN_GROUP_SYNC_EVERY_STATUS_REFRESHES = 5/)
assert.match(embed, /refreshStatusesWithLatestTokenGroups/)
assert.match(embed, /const scopeAvailable = !nextGroupFilter\.startsWith\('token:'\)/)
assert.match(embed, /setTokenGroupScopeUnavailable\(!scopeAvailable\)/)
assert.match(embed, /if \(!scopeAvailable\) clearTrustedStatuses\(\)/)
assert.match(embed, /if \(!tokenGroupSync\.scopeAvailable\) return/)
assert.doesNotMatch(embed, /nextGroupFilter = 'all'/)
assert.match(embed, /function embedStatusScopeKey\(window: string, filter: string, models: readonly string\[\]\)/)
assert.match(embed, /statusWindow === timeWindow && statusScopeKey === desiredStatusScopeKey \? modelStatuses : \[\]/)
assert.match(embed, /const requestScopeKey = embedStatusScopeKey\(timeWindow, scopeFilter, fetchSet\)/)
assert.match(embed, /if \(filter !== groupFilterRef\.current\) clearTrustedStatuses\(\)/)
assert.match(embed, /configState === 'ready'[\s\S]*?statusScopeKey !== desiredStatusScopeKey/)
assert.match(embed, /const restoreTriggerFocus = useCallback\(\(\) => \{[\s\S]*?trigger\?\.isConnected[\s\S]*?trigger\.focus\(\)/)
...
assert.match(userManagement, /total_users: Math\.max\(0, prev\.total_users - 1\)/)
assert.ok(
