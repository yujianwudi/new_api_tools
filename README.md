...
<p align="center">
  <img alt="Version" src="https://img.shields.io/badge/version-v0.6.2-2563EB?style=for-the-badge" />
...
| 发票证据 | 在 Tool Store 保存蓝票/红票文档与只追加事件，按币种和精度统计有效蓝票、有效红票、作废与净已开票金额 |
...
- 单张发票使用正 `int64 amount_minor` 与 `tax_amount_minor`，税额不得大于含税金额，并显式保存三字母 `currency` 和 `0..9` 的 `minor_unit_scale`；
- 服务端汇总使用 Go `math/big` 任意精度整数，同一分组累计值可以超过 `int64`；summary、审计和事件中的金额一律作为十进制字符串返回或记录；
- 红票金额保持正数并通过 `related_invoice_number` 关联原蓝票；净已开票金额等于有效蓝票金额减去有效红票金额；
- 汇总严格按 `currency + minor_unit_scale` 分组，不跨币种相加；
- 单张录入和 CSV 导入都经过 RBAC、幂等键、事务与操作审计保护。

第一版只统计有效蓝票金额、有效红票金额、作废金额、净已开票金额和票数。它不关联 `top_ups` 或 `subscription_orders`，不处理退款、拒付和申请占用，因此**不会展示可开票收入、剩余可开票金额或开票率**。蓝票和红票记录只能证明“授权操作员录入了这份证据”，不能替代税务平台开票或验真。

控制台提供一级导航 `/invoices`。页面按币种分别展示有效蓝票、有效红票冲减、作废、净已开票和票数，支持币种、状态、Asia/Shanghai 开票日期半开区间筛选和游标分页。金额响应使用十进制字符串，前端使用 `BigInt` 精确格式化；请求失败时保留上次成功数据并标记陈旧，不会把不可用冒充成 0。后端未提供异常指标时页面明确显示“未提供”。

## NewAPI 版本与写入限制

v0.6.2 的已验证控制面契约基线是 **NewAPI `v1.0.0-rc.21`**。
...
| 角色 | 权限 |
|---|---|
| `viewer` | 只读查询、健康诊断、审计，以及发票汇总和脱敏列表查看 |
| `operator` | viewer 权限及常规运营写操作、发票完整详情、录入和 CSV 导入 |
| `admin` | operator 权限及永久删除、价格快照、发票作废等高风险操作 |
...
- `GET /api/invoices/summary?currency=&issued_from=&issued_to=`
- `GET /api/invoices?status=&currency=&issued_from=&issued_to=&cursor=&limit=`
- `GET /api/invoices/:id`
- `POST /api/invoices`
- `POST /api/invoices/import/preview`
- `POST /api/invoices/import/confirm`
- `POST /api/invoices/:id/void`

发票汇总和脱敏列表至少要求 `viewer`；完整详情、创建、导入预览和确认要求 `operator`；作废要求 `admin`。每个响应都返回发票 capabilities，前端在能力未知时默认隐藏详情和写操作。列表会脱敏购方姓名和税号，CSV preview 也不会回显这些字段；系统不会把结构化 `buyer_name`、`buyer_tax_id` 自动复制到通用操作审计或发票事件。
...
除只读 preview 外，所有发票写入都要求 `Idempotency-Key`，并在同一个 Tool Store 事务中写入文档/事件和操作审计。金额响应为十进制字符串；CSV 必须是 UTF-8、最多 1 MiB/500 行，并先 preview 再 confirm，非法金额、币种、日期、重复票号和公式前缀会被明确拒绝。v0.6.2 仍不提供发票导出 API。
...
Tool Store 及其备份会包含购方名称和可选税号快照，必须按财务 PII 保护：文件权限至少 `0600`，使用加密磁盘或等价静态加密、受控备份、最小权限访问和明确留存/销毁策略。结构化购方字段不会自动进入通用操作审计、发票事件、访问日志或浏览器 pending marker；自由文本理由仍必须由操作员主动避免写入 PII。
...
v0.6.2 遇到网络中断、上游超时或审计尾部失败时，不会根据页面当前状态猜测结果，也不会盲目重放原请求。浏览器会保留无敏感负载的 pending marker，并通过 `GET /api/control-plane/operations/:idempotency_key` 读取与当前 actor、认证方式、动作和目标绑定的完整 intent/outcome 链；只有可证明的终态才能释放本地提交锁，损坏或孤立审计链返回 `503` 并继续失败关闭。
...
v0.6.2 继续缩小自动化写入面。下面的旧能力不得视为可用的自动风控：
...
| 批量开启 IP 记录 | 返回 501 |
| Storage 通用配置入口（GET/POST/DELETE） | 全部返回 501；v0.6.2 不提供通用配置读取或写入 |
| 自动创建数据库索引 | 禁用，只列出建议 |
...
安装器 commit 与脚本 SHA-256 已固定；执行前仍必须从 v0.6.2 发行页复制真实的 manifest digest 与 release commit，并替换剩余全部 `REPLACE_WITH_*`。任一占位符未替换时不要执行。
...
NEWAPI_TOOLS_RELEASE_TAG=v0.6.2 \
bash ./deploy.sh
```
...
| `NEWAPI_TOOLS_RELEASE_TAG` | 镜像签名与 provenance 必须匹配的严格发行标签 | 显式发行 digest 时必填，如 `v0.6.2` |
...
```bash
backup_dir="backups/v0.6.2-$(date +%Y%m%d%H%M%S)"
mkdir -p "$backup_dir"
...
依赖诊断需要 JWT 或 API Key；`/metrics` 需要独立观测 token。更完整的升级、回滚和兼容性说明见 [`RELEASE_0.6.2.md`](./RELEASE_0.6.2.md)。
...
- v0.6.2 供应链恢复任务书与验收标准：[`docs/V0.6.2_SUPPLY_CHAIN_RECOVERY_TASK_BOOK.md`](./docs/V0.6.2_SUPPLY_CHAIN_RECOVERY_TASK_BOOK.md)
- v0.6.2 发行说明：[`RELEASE_0.6.2.md`](./RELEASE_0.6.2.md)
- v0.6.1 整改任务书（运行时整改基线，发行由 v0.6.2 接续）：[`docs/V0.6.1_REMEDIATION_TASK_BOOK.md`](./docs/V0.6.1_REMEDIATION_TASK_BOOK.md)
